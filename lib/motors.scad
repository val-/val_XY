...
)
{
	...
	if (mochup == true)
	{
		...
		{
			color("grey") cube(size=[bolt_hole_distance+bolt_hole_size+5,bolt_hole_distance+bolt_hole_size+5,motor_length], center = true);
		}
		//shaft
		translate(v=[0,0,-(motor_length-motor_shaft_length-2)/2])
		{
			color("LightGrey") cylinder(r=motor_shaft_diameter/2,h=motor_length+motor_shaft_length--1, center = true);
		}
	}
	}
}
